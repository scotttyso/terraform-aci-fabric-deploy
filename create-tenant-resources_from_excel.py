...
def port_static_path(pod, switch_ipr, node_id, int_select, port, epg, vlan, vl_mode, afile):
    afile.write('resource "aci_epg_to_static_path" "%s_%s_%s" {' % (switch_ipr, int_select, epg))
    afile.write('\tdepends_on           = [aci_application_epg.%s,aci_range.st_vlan_pool_%s]' % (epg, vlan))
    afile.write('\tapplication_epg_dn   = aci_application_epg.%s.id' % (epg))
    afile.write('\ttdn                  = "topology/pod-%s/paths-%s/pathep-[%s]"' % (pod, node_id, port))
    afile.write('\tencap                = "vlan-%s"' % (acc_vlan))
    afile.write('\tmode                 = "native"' % (vl_mode))
    afile.write('}')
...
def resource_access_pg(switch_ipr, int_select, node_id, pg_name, aep, mtu, speed, swpt_mode, acc_vlan, trunk_vlans, cdp, lldp, bpdu, descr):
    ...
    file_stbind = './tenant/resources_user_static_bindings_%s' % (switch_ipr)
...
def resource_bds(add_type, tenant, vrf, bd_name, extend, descr):
    arpf = '%s' % (extend)
    if extend == 'yes':
        optimiz_w = 'no'
        mdest = 'bd-flood'
    ...
    if add_type == 'bd_nca':
        wr_file = wr_epgs
        wr_file.write('resource "aci_application_epg" "fooapplication_epg" {\n')
        ...
        wr_file.write('\tname                   = "demo_epg"\n')
        wr_file.write('\tdescription            = "%s"\n' % (descr))
        wr_file.write('\tannotation             = "tag_epg"\n')
        wr_file.write('\texception_tag          = "0"\n')
        ...
        wr_file.write('\tname_alias             = "alias_epg"\n')
        wr_file.write('\tpc_enf_pref            = "unenforced"\n')
        wr_file.write('\tpref_gr_memb           = "exclude"\n')
...
for r in ws3.rows:
    if any(r):        
        ...
        if type == 'vrf_add':
            ...
            print(vrf_name, vrf_desc,fltr_type)
...
for r in ws8.rows:
    if any(r):
        ...
        if type == 'add_apg':
            ...
            mtu = str(r[7].value)
            speed = str(r[8].value)
            swpt_mode = str(r[9].value)
            acc_vlan = str(r[10].value)
            trunk_vlans = str(r[11].value)
            cdp = str(r[12].value)
            lldp = str(r[13].value)
            bpdu = str(r[15].value)
            descr = str(r[17].value)
            if cdp == 'no':
                cdp = 'cdp_Disabled'
            else:
                cdp = 'cdp_Enabled'
            ...
            # Create Resource Records for Tenant
            resource_access_pg(switch_ipr, int_select, node_id, pg_name, aep, mtu, speed, swpt_mode, acc_vlan, trunk_vlans, cdp, lldp, bpdu, descr)
            line_count += 1
        elif type == 'tnt_vrf':
            tenant_name = str(r[1].value)
            tenant_desc = str(r[2].value)
